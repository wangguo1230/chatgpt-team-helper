...
import { markAccountAsInvalid } from '../src/services/account-sync.js'
...
const seedData = (db) => {
  db.run(`
    ...
    VALUES (
      1, 'invalid-test@example.com', 'token-1', NULL, 0, 0, 'acct-1', 'device-1',
  ...
  db.run(`
    ...
    VALUES
      ('CODE-UNUSED-1', 0, 'invalid-test@example.com'),
      ('CODE-UNUSED-2', 0, 'invalid-test@example.com'),
      ('CODE-USED-1', 1, 'invalid-test@example.com')
  `)
}

test('markAccountAsInvalid 重复触发时仅首次发送失效邮件', async () => {
  ...
  const first = await markAccountAsInvalid(db, 1, 'unit test first invalid', {
    sendAlertEmail: true,
    alertEmailSender
  })
  const second = await markAccountAsInvalid(db, 1, 'unit test repeated invalid', {
    sendAlertEmail: true,
    alertEmailSender
  })

  assert.equal(first.newlyInvalid, true)
  assert.equal(first.alertEmailSent, true)
  assert.equal(first.deletedUnusedCodeCount, 2)

  assert.equal(second.newlyInvalid, false)
  assert.equal(second.alertEmailSent, false)
  assert.equal(second.deletedUnusedCodeCount, 0)

  assert.equal(alertPayloads.length, 1)
  assert.match(String(alertPayloads[0]?.subject || ''), /\[账号失效\] invalid-test@example\.com/)

  const accountResult = db.exec(`
    SELECT is_open, expire_at
    FROM gpt_accounts
    WHERE id = 1
  `)
  ...
  assert.equal(accountRow[1], '1970/01/01 00:00:00')

  const unusedCountResult = db.exec(`
    SELECT COUNT(*)
    FROM redemption_codes
    WHERE account_email = 'invalid-test@example.com'
      AND is_redeemed = 0
  `)
